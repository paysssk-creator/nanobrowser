...
def llmcall(pid,prompt,cb):
 p=LLM.get(pid)
 if not p:return cb("Provider not found")
 def rn():
  try:
   if pid=="ollama":
  ...
  except Exception as e:cb("Err:"+str(e)[:80])
 threading.Thread(target=rn,daemon=True).start()
class NB:
 def __init__(self):
 ...
 def _ui(self):
  ...
  for t,cmd in[("◀",self.gb),("▶",self.gf),("↻",self.rl),("⌂",self.gh)]:tk.Button(nf,text=t,width=3,command=cmd,**bs).pack(side=tk.LEFT,padx=1)
 ...
 def _rl(self):
  ...
  for i,kv in enumerate(LLM.items()):
   ...
   tk.Label(f,text="●"if v["ok"]else"○",bg=f["bg"],fg=c,font=("Segoe UI",12)).pack(side=tk.LEFT,padx=(8,4))
  ...
  def _lt(self,k):self.l_o.insert(tk.END,chr(10)+"[Test "+k+"]"+chr(10));self.l_o.see(tk.END);llmcall(k,"Hello!",lambda t:self.l_o.insert(tk.END,t+chr(10)))
  def _ls(self):
  ...
  def _ba(self):
   ...
   tk.Label(hd,text="A-Share Market",bg=C1,fg=FG,font=("Segoe UI",11,"bold")).pack(side=tk.LEFT)
  ...
  def _rh(self):
   ...
   for c in CAPS:
    ...
    tk.Button(row,text=c[:16],bg=C2,fg=FG,font=("Segoe UI",7),bd=1,relief="solid",cursor="hand2",activebackground=AC,command=lambda cid=c:self._hx(cid)).pack(side=tk.LEFT,padx=1,pady=1);col+=1
  def _hx(self,cid):self.hl.insert(1.0,"Run: "+cid+chr(10));ap("/api/hacker/exec",{"id":cid,"action":"run"},lambda d:self.hl.insert(1.0,"["+("OK"if d.get("ok")else"FAIL")+"] "+cid+chr(10)))
  def _bm(self):
   p=self.p["m"];hd=tk.Frame(p,bg=C1,height=28);hd.pack(fill=tk.X,padx=8,pady=(8,4));hd.pack_propagate(False)
   tk.Label(hd,text="MCP 19 Servers",bg=C1,fg=FG,font=("Segoe UI",11,"bold")).pack(side=tk.LEFT)
  ...
  def _bk(self):
   ...
   tk.Label(hd,text="Desktop Control",bg=C1,fg=FG,font=("Segoe UI",11,"bold")).pack(side=tk.LEFT)
  ...
  def _rk(self):pass
  def _kx(self,cid):self.kl.insert(1.0,"Run: "+cid+chr(10));ap("/api/hacker/exec",{"id":cid,"action":"run"},lambda d:self.kl.insert(1.0,"["+("OK"if d.get("ok")else"FAIL")+"] "+cid+chr(10)))
  def _tk(self):
  ...
  def _up(self,d):
   ...
   self.d_mc.config(text=m or"No MCP");self.stl.config(text=str(len(srvs))+" MCP online")
  def gb(self):
